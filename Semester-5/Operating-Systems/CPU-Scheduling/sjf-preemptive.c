...
Process* init(int);
void sjf_preemptive(Process *, int);
...
int main(int argc, char* argv[]) {
    ...
    sjf_preemptive(processes, 4);
    print_table(processes, 4);

    free(processes);
}

void sjf_preemptive(Process* processes, int number_of_processes) {
    int completed = 0;
    int time = 0;
    ...
    minimum_burst_time = 1000 + (rand() % 1000);
    while(completed != number_of_processes) {
        ...
        for(int i = 0; i < number_of_processes; i++) {
            ...
            if((time >= processes[i].arrival_time) && (minimum_burst_time > processes[i].burst_time)) {    
        ...
        if(flag == 1 || index == prev_index) {
            ...
            minimum_burst_time = remaining_time[index];
            prev_index = index; 
        ...
        time++;
        minimum_burst_time = (remaining_time[index] == 0) ? rand() : remaining_time[index]; 
    }
    free(remaining_time);
